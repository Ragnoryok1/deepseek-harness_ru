...
export const ru: Record<AgentPresetSettingsKey, string> = {
  title: 'Пресет агента',
  description: 'Применяется к сессиям, которые вы начнёте впредь. Работающие сессии сохраняют пресет, с которым начались.',
  ...
  nav: 'Пресеты агента',
  sectionIntro:
    'Пресет — это композиция плагинов, с которыми работает агент сессии: его инструменты, промпт и возможности. '
    + 'Скопируйте существующий пресет и настройте под себя или позвольте агенту составить его в режиме Creator.',
  builtIn: 'Встроенные',
  setDefault: 'Сделать пресетом по умолчанию',
  view: 'Просмотр',
  presetStandardName: 'Стандартный режим',
  presetStandardDescription:
    'Полноценный агент-программист: редактирование файлов, shell, поиск по файлам и в вебе, навыки, планирование, цели, субагенты и воркфлоу.',
  presetCodeName: 'Режим кода',
  presetCodeDescription:
    'Все возможности стандартного режима, а инструменты доступны через Code Mode SDK — модель может объединять многошаговые операции в одну программу на TypeScript.',
  presetMinimalName: 'Минимальный режим',
  presetMinimalDescription:
    'Агент-программист из двух инструментов: постоянный bash и str_replace_editor.',
  presetCordisName: 'Режим Creator',
  presetCordisDescription:
    'Создан для разработки собственных пресетов: все возможности стандартного режима плюс инспекция на лету, эксперименты с плагинами и подсказки по созданию пресетов.',
  ...
  copyIntro:
    'Пресет целиком копируется на этом компьютере. Идентификатор становится именем каталога и не меняется впоследствии; '
    + 'всё остальное редактируется в файлах самого пресета.',
  create: 'Создать',
  creating: 'Создание…',
  ...
  deleteDescription:
    'Каталог пресета будет удалён. Сессии, которые уже работают на нём, продолжат работу; новые сессии не смогут его выбрать.',
