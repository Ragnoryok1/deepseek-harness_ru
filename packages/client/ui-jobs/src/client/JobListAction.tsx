...
export function JobListAction({ sessionId, useSessions, t }: JobListActionProps) {
  ...
  const category = pluralCategory(live ? liveCount : jobs.length)
  const countKey = live
    ? category === 'one' ? 'count.live.one' : category === 'few' ? 'count.live.few' : 'count.live.other'
    : category === 'one' ? 'count.idle.one' : category === 'few' ? 'count.idle.few' : 'count.idle.other'
